...
AUM_MIN             = 100_000_000    # 100M€ AUM minimo
MIN_AGE_MONTHS      = 36     # Minimo 3 anni di track record
...
def analizza_etf(ticker: str, nome_noto: str, aum: float, portfolio_returns: dict[str, pd.Series]) -> dict | None:
    try:
        ...
        # Se Yahoo non restituisce ISIN, lascia vuoto
        if not isin:
            isin = ""
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*65}")
    print(f"SCAN COMPLETATO — {len(risultati)} ETF hanno superato i filtri")
    print(f"{'='*65}")